...
const IntegralService = () => {
  return (
    <section className="py-20 md:py-28 bg-slate-dark">
      <div className="container max-w-5xl mx-auto px-4 text-center">
        <h2 className="font-heading text-3xl md:text-4xl font-bold text-primary-foreground mb-4">
          Servicio <span className="text-gold">Integral</span>
        </h2>
        <div className="w-16 h-[2px] bg-gold mx-auto mb-8" />
        <p className="font-body text-primary-foreground/70 text-lg max-w-3xl mx-auto mb-14 leading-relaxed">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {services.map((s) => (
            <div key={s.label} className="flex flex-col items-center gap-4">
              ...
              <span className="font-body text-sm text-primary-foreground/80 font-medium">
                {s.label}
              </span>
            </div>
          ))}
